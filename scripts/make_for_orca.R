...
parse_orca_output<-function(data, refdata, category, getweight=F, traindata=NA) {
    ...
    colnames(records_frame)<-c("id","score","neighbors")
    ...
    if (getweight){
        res=scores[,c("name","category","score","neighbors")]
        ...
        return(scores[,c("name","category","score","neighbors")])
    }
}
...
#foreach(fold=1:n.folds,.packages=c("stringr"))%:%{
    ...
    #cat(paste0(orca_dir,"bin"))
        ...
        #get train data scores
            ...
            orca_args=c(command,train_file,train_file,weights_file,orca_disttype,"-rn","-k",orca_knn-1,"-n",nrow(ref_train),dtype,orca_use_weights)
            ...
            scores_all=setNames(rbind(scores_all,train_scores),c("name","category","score","neighbors"))
            #cat(category,"=1\n")
            ref_test_file=sub("_train\\.","_test\\.",sub("/bin/","/pre/",sub("bin$","ref",train_file)))
            ref_test=read.table(file=ref_test_file,head=T,sep=";")
            orca_args=c(command,test_file,train_file,weights_file,orca_disttype,"-rn","-k",orca_knn,"-n",nrow(ref_test),dtype,orca_use_weights)
            ...
            res=list(category=scores_all[,c("name","score","neighbors")])
            names(res)<-category
